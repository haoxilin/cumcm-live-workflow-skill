...
| 获奖范文 / 参考解答（2~3 篇） | 强烈建议 | 答案数值校准与写作风格参考（模拟训练时） |
| AI 自查表（标准版） | 终审阶段 | 26 条六维度逐项核查清单 |
| 写作方法指导（如韩中庚） | 可选 | 论文结构规范参考 |

**推荐的开场指令（示例）**：

> 按 cumcm-live-workflow 技能执行。材料在 `2024A题/` 目录：原题 `A题.pdf`、格式规范 `格式规范.pdf`、附件 `附件/`、范文 `范文_A053.txt`。请从读题自查开始，走完全流程。
...
- **v7.3**：README 增加"使用前准备：喂给 AI 的初始材料"清单（赛题/格式规范/附件/AI 规定/范文/自查表等）与推荐开场指令
