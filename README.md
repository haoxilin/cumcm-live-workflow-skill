...
└── references/                       # 可执行验证模板
    ├── verify_figure_arrows.py       #   流程图箭头穿框检测（间隙条带法）
...
- **v5.3 · 配套模板与工作示例（当前）**：新增可执行验证模板（references/ 箭头穿框检测/xlsx校验/PDF体检/敏感性跑批）、最小工作示例 WORKED_EXAMPLE.md（水箱注水全链路）、参考材料 templates/（格式规范/AI规定/自查表/Word模板）挂载并说明用法
- **v5.2 · 成品结构修复**：新增论文成品结构模板（完整章节目录，每问固定三小节）、消除摘要规则矛盾、确立编号规则唯一权威
- **v5.1 · 启动关键词与环境自动就绪**：三类触发词、自动环境检测与完成度清单、确认后自动安装
- **v5 · 国奖级指标与图表中文化**：顶刊级图表规范（中文标签/统一配色/正交流程图/箭头止于框边）、五段式摘要、文献与公式扩充标准、降重四层法与九类 AI 痕迹自查、国奖级硬性指标清单
